...
abstract contract SMARTCappedTest is AbstractSMARTTest {
    ...
    function test_Capped_MintExactlyAtCap_Success() public {
        ...
        assertEq(token.balanceOf(clientBE), initialBalance + remainingCapacity, "Balance should increase by remaining capacity");
    ...
    function test_Capped_SupportsInterface() public {
        ...
        assertTrue(
            IERC165(address(token)).supportsInterface(cappedInterfaceId),
            "Should support ISMARTCapped interface"
        );
        
        // Test ERC165 interface support
        bytes4 erc165InterfaceId = type(IERC165).interfaceId;
        assertTrue(
            IERC165(address(token)).supportsInterface(erc165InterfaceId),
